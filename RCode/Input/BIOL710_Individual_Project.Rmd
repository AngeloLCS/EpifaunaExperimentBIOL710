...
For my statistical analysis assignment, I will focus on my second aim. Below is some of the work I have completed to simulate the data values that I can expect during my experiments. More information on my experiments is available in the dataset card available here: https://github.com/AngeloLCS/EpifaunaExperimentBIOL710/blob/070cc3c0aeba8b0e9b3f4680e01527374d516e12/RCode/Input/DatasetCard.md 
...
```{r}
# Importing Data
epifauna <- read.csv('epifauna.csv')
str(epifauna)
```
...
gamm_glm <- glm(gamm_loss ~ treatment*fish_mass_tot, family = "poisson", data = epifauna)
summary(gamm_glm)

capr_glm <- glm(capr_loss ~ treatment*fish_mass_tot, family = "poisson", data = epifauna)
summary(capr_glm)

pent_glm <- glm(pent_loss ~ treatment*fish_mass_tot, family = "poisson", data = epifauna)
summary(pent_glm)
```
...
```{r}
#Figure
gamm_f2 <- ggplot(epifauna, aes(x = treatment, y = gamm_loss)) +
...
capr_f2 <- ggplot(epifauna, aes(x = treatment, y = capr_loss)) +
  geom_boxplot(alpha = 0.6) +
  labs(
    x = "Treatment",
...
pent_f2 <- ggplot(epifauna, aes(x = treatment, y = pent_loss)) +
  geom_boxplot(alpha = 0.6) +
  labs(
    x = "Treatment",
    y = "Loss of Pentidotea") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
pent_f2
```
